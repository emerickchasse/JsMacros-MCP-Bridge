...
dependencies {
    ...
    add(mcpRuntime.name, libs.mcp)
    add(mcpRuntime.name, libs.tomcat)

    // MCP runs inside the client mod and is bundled into the produced Fabric jar.
    add("fabricImplementation", libs.mcp)
    add("fabricImplementation", libs.tomcat)
    ...
    fabricModImplementation(libs.cloth.config) {
...
val fabricJar by tasks.getting(Zip::class) {
    ...
    // The official MCP Java SDK is not a Fabric mod. Flatten its complete runtime
    // graph (SDK, JSON mapper, Reactor and embedded Tomcat) into our mod jar.
    from(provider { mcpRuntime.resolve().map { zipTree(it) } }) {
...
fun modrinthChangelog(): String = """
    JsMacros MCP Bridge ${project.version} for Fabric on Minecraft $mcVersion.

    Full changelog: https://github.com/emerickchasse/JsMacros-MCP-Bridge/releases/tag/v${project.version}
""".trimIndent()
...
publishMods {
    ...
    if (publishModrinth) {
        modrinth("modrinthFabric") {
            ...
            displayName.set("JsMacros MCP Bridge ${project.version} (Fabric $mcVersion)")
